...
// hairStyle 0..12: 0短髪 1丸刈り 2アフロ 3フラットトップ 4ヘッドバンド 5ロング(サイド長め)
...
/** 見た目の番号3つ組 → 色/髪型（PlayerLook）。playerdb 由来の見た目はこの経路。 */
export function resolveLook(idx: LookIdx): PlayerLook {
  ...
  return { skinHex, hairHex, skin: hexRGB(skinHex), hair: hexRGB(hairHex), style: idx[2] };
}
...
// 名前ハッシュから決定的ランダムな髪型を割り当てる。名は playerdb と完全一致が必要。
...
// ランダム（オーバーライドされない）選手の間での各髪型の相対頻度。大半は同じ(1.0)。
...
/** 名前ハッシュから見た目の番号3つ組を導く（DB未登録の選手 / playerdb 生成用）。 */
